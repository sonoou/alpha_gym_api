...
public class ApiDocsController {
    ...
    public ResponseEntity<Map<String, Object>> getApiDocumentation() {
        ...
        endpoints.add(createEndpoint("Payments", "GET", "/api/payment/history", "Get user's previous payment transactions history sorted by date DESC with receipt download URLs", true, null, List.of(Map.of("id", 1, "razorpayOrderId", "order_MZk123456", "razorpayPaymentId", "pay_MZn876543", "amount", 499.0, "currency", "INR", "planName", "Monthly Starter Plan", "status", "SUCCESS", "paymentDate", "2026-08-14T16:20:00", "receiptDownloadUrl", "/api/payment/receipt/1/download"))));
        endpoints.add(createEndpoint("Payments", "GET", "/api/payment/receipt/{transactionId}", "View/stream generated payment receipt PDF (Alpha Veins Gym template)", true, null, "PDF binary stream (application/pdf)"));
        endpoints.add(createEndpoint("Payments", "GET", "/api/payment/receipt/{transactionId}/download", "Download generated payment receipt PDF file", true, null, "PDF file download (attachment)"));
        endpoints.add(createEndpoint("Payments", "GET", "/api/payment/receipt/order/{orderId}", "Download payment receipt PDF by Razorpay Order ID", true, null, "PDF file download (attachment)"));
        ...
        // 7. Workouts & Exercise Library (695+ Workouts with Pagination)
        endpoints.add(createEndpoint("Workouts", "GET", "/api/workouts", "Get paginated workouts (params: ?page=0&size=20&category=Chest&difficulty=Beginner&search=bench&sortBy=id&sortDirection=asc)", false, null, Map.of("content", List.of(Map.of("id", 142, "name", "Barbell Bench Press", "category", "Chest", "difficulty", "Advanced", "targetMuscles", "Pectoralis Major, Triceps")), "page", 0, "size", 20, "totalElements", 695, "totalPages", 35, "last", false)));
        ...
        endpoints.add(createEndpoint("Workouts", "GET", "/api/workouts/all", "Get all non-paginated workouts list (optional ?category=Chest, ?difficulty=Beginner)", false, null, "Full list of Workout objects"));
        endpoints.add(createEndpoint("Workouts", "GET", "/api/categories", "Get all 10 workout categories (Neck, Shoulder, Chest, Biceps, Triceps, Abs, Forearms, Quads, Calves, Back)", false, null, List.of(Map.of("id", 1, "name", "Neck"), Map.of("id", 3, "name", "Chest"), Map.of("id", 4, "name", "Biceps"), Map.of("id", 10, "name", "Triceps"))));
    ...
    public ResponseEntity<Map<String, Object>> getOpenApiJson() {
        ...
        paths.put("/api/payment/history", Map.of("get", createOperation("Get Payment History", "Get user's previous payment transactions history with receipt download URLs", true, null, Map.of("200", createResponse("Payment History List", List.of(Map.of("id", 1, "razorpayOrderId", "order_MZk123456", "razorpayPaymentId", "pay_MZn876543", "amount", 499.0, "currency", "INR", "planName", "Monthly Starter Plan", "status", "SUCCESS", "paymentDate", "2026-08-14T16:20:00", "receiptDownloadUrl", "/api/payment/receipt/1/download")))))));
        paths.put("/api/payment/receipt/{transactionId}", Map.of("get", createOperation("View Receipt PDF", "Stream payment receipt PDF in browser (Alpha Veins Gym template)", true, null, Map.of("200", createResponse("Receipt PDF stream", "application/pdf")))));
        paths.put("/api/payment/receipt/{transactionId}/download", Map.of("get", createOperation("Download Receipt PDF", "Download payment receipt PDF file attachment", true, null, Map.of("200", createResponse("Receipt PDF attachment", "application/pdf")))));
        paths.put("/api/payment/receipt/order/{orderId}", Map.of("get", createOperation("Download Receipt by Order ID", "Download payment receipt PDF file by Razorpay Order ID", true, null, Map.of("200", createResponse("Receipt PDF attachment", "application/pdf")))));
        ...
        // Workouts & Categories
        paths.put("/api/workouts", Map.of("get", createOperation("Get Paginated Workouts", "Get workouts with pagination & filters (query params: page, size, category, difficulty, search, sortBy, sortDirection)", false, null, Map.of("200", createResponse("Paginated Workouts", Map.of("content", List.of(Map.of("id", 142, "name", "Barbell Bench Press", "category", "Chest", "difficulty", "Advanced")), "page", 0, "size", 20, "totalElements", 695, "totalPages", 35, "last", false))))));
        paths.put("/api/workouts/all", Map.of("get", createOperation("Get All Workouts List", "Get non-paginated full list of workouts", false, null, Map.of("200", createResponse("Workouts List", List.of(Map.of("id", 142, "name", "Barbell Bench Press")))))));
        ...
        paths.put("/api/categories", Map.of("get", createOperation("Get Categories List", "Get all 10 workout categories (Neck, Shoulder, Chest, Biceps, Triceps, Abs, Forearms, Quads, Calves, Back)", false, null, Map.of("200", createResponse("Categories List", List.of(Map.of("id", 1, "name", "Neck"), Map.of("id", 3, "name", "Chest"), Map.of("id", 4, "name", "Biceps"), Map.of("id", 10, "name", "Triceps")))))));
